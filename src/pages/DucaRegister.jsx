...
export default function DucaRegister({ token }) {
  ...
  const openToast = (type, message) => setToast({ open: true, type, message });

  const onSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await registrarDUCA(token, form);
      openToast("success", `Declaración registrada (${res.numeroDocumento})`);
      // opcional: reset del número para el siguiente registro
      setForm((f) => ({ ...f, numeroDocumento: "" }));
    } catch (err) {
      openToast("error", String(err));
    }
  };
      ...
      <div className="card">
        ...
        <form onSubmit={onSubmit} className="grid md:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <select className="input" value={form.moneda} onChange={(e) => update("moneda", e.target.value)}>
              <option value="USD">USD</option>
              <option value="QTZ">QTZ</option>
              <option value="EU">EU</option>
              {/* Si decides usar ISO 4217: <option value="GTQ">GTQ</option><option value="EUR">EUR</option> */}
            </select>
          </div>
