...
export default function DucaRegister({ token }) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      setToast({ open:true, type:"success", message:`Declaración registrada (${res.numeroDocumento})` });
      // opcional: limpiar número para siguiente registro
      // setForm(f => ({ ...f, numeroDocumento: "" }));
    } catch (err) {
      setToast({ open:true, type:"error", message:String(err) });
    }
  };
        ...
        <form onSubmit={onSubmit} className="grid md:grid-cols-2 gap-3">
          <div><label className="label">Número</label>
            <input className="input" value={form.numeroDocumento}
              onChange={e=>update("numeroDocumento", e.target.value)} required /></div>

          <div><label className="label">Fecha emisión</label>
            <input className="input" type="date" value={form.fechaEmision}
              onChange={e=>update("fechaEmision", e.target.value)} required /></div>

          <div><label className="label">País emisor</label>
            <input className="input" value={form.paisEmisor}
              onChange={e=>update("paisEmisor", e.target.value)} /></div>
          ...
          <div><label className="label">Valor aduana total</label>
            <input className="input" type="number" value={form.valorAduanaTotal}
              onChange={e=>update("valorAduanaTotal", Number(e.target.value))} /></div>
          ...
          <div><label className="label">Nombre</label>
            <input className="input" value={form.importador.nombre}
              onChange={e=>update("importador.nombre", e.target.value)} /></div>
          <div><label className="label">Documento</label>
            <input className="input" value={form.importador.documento}
              onChange={e=>update("importador.documento", e.target.value)} /></div>
          ...
          <div><label className="label">Nombre</label>
            <input className="input" value={form.exportador.nombre}
              onChange={e=>update("exportador.nombre", e.target.value)} /></div>
          <div><label className="label">Documento</label>
            <input className="input" value={form.exportador.documento}
              onChange={e=>update("exportador.documento", e.target.value)} /></div>
          ...
          <div><label className="label">Medio</label>
            <input className="input" value={form.transporte.medio}
              onChange={e=>update("transporte.medio", e.target.value)} /></div>
          <div><label className="label">Placa</label>
            <input className="input" value={form.transporte.placa}
              onChange={e=>update("transporte.placa", e.target.value)} /></div>
