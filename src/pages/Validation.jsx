...
export default function Validation() {
  ...
  async function loadData() {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_BASE}/validacion/pendientes`);
  ...
  async function accion(numero, aprobar = true) {
    try {
      const res = await fetch(`${API_BASE}/validacion/${aprobar ? "aprobar" : "rechazar"}`, {
        method: "POST",
                ...
                items.map((it) => (
                  <tr key={it.numero}>
                    ...
                    <td className="space-x-2">
                      <button className="btn btn-primary" onClick={() => accion(it.numero, true)}>
                        Aprobar
                      </button>
                      <button className="btn btn-outline" onClick={() => accion(it.numero, false)}>
                        Rechazar
                      </button>
