...
/** Intenta obtener detalle de varias rutas posibles. Degrada con gracia. */
async function fetchDucaDetail(numero) {
  ...
  // si nada funcionó, devolvemos algo mínimo
  return null;
}
...
function DetailModal({ open, onClose, baseItem }) {
  ...
  // si el backend devolvió un objeto, añadimos pares clave/valor amigables
  if (detail && typeof detail === "object") {
    Object.entries(detail).forEach(([k, v]) => {
      if (v == null || String(v).trim() === "") return;
      // evitamos duplicar las claves que ya mostramos arriba
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
      <div className="w-full max-w-2xl rounded-2xl bg-zinc-900 text-zinc-100 shadow-xl ring-1 ring-white/10">
        ...
        <div className="px-5 py-4">
          ...
          {loading && (
            <p className="mt-4 text-sm text-zinc-400">Cargando detalle…</p>
          )}
          {!loading && !detail && (
            <p className="mt-4 text-sm text-zinc-400">
              No hay información adicional disponible para este número.
            </p>
          )}
        </div>
  ...
  useEffect(() => {
    const fetchPendientes = async () => {
      try {
        ...
        "https://aduanas-duca-api.onrender.com/validacion/pendientes"
        );
        const json = await res.json();
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {data.length === 0 ? (
              <tr>
                <td colSpan="4" className="py-6 text-center text-red-300">
                  Sin registros
                </td>
              </tr>
            ) : (
                ...
                return (
                  <tr key={i} className="border-b border-white/5 hover:bg-white/5">
                    ...
                    <td className="py-3 space-x-2">
                      <button className="px-3 py-1.5 rounded-lg bg-cyan-600 hover:bg-cyan-500 text-zinc-50">
                        Aprobar
                      </button>
                      <button className="px-3 py-1.5 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-zinc-200 ring-1 ring-white/10">
                        Rechazar
                      </button>
                      <button
                        className="px-3 py-1.5 rounded-lg bg-emerald-700/30 hover:bg-emerald-700/40 text-emerald-200 ring-1 ring-emerald-600/30"
                        onClick={() => openWith(item)}
                      >
                        Ver detalle
                      </button>
